class Rsc_ZEI_ListBuildings
{
	...
	class ZEI_LB_Slider_MinTemplates: RscSlider
	{
		...
		h = 0.033 * safezoneH;
		onSliderPosChanged = "if (round (_this select 1) > 0) then { (findDisplay 1703 displayCtrl 1) ctrlSetText format['(%1 or lower)', round (_this select 1)]; } else { (findDisplay 1703 displayCtrl 1) ctrlSetText '(No Template)'; };";
		onLoad= "_this spawn {\
			waitUntil { !isNull (_this select 0) };\
